...
subroutine evol(infile,logfile,evfile,dumpfile)
 ...
 integer         :: noutput,noutput_dtmax,nsteplast,ncount_fulldumps,io,n
 ...
 np_cs_eq_0 = 0
 np_e_eq_0  = 0
 ! n_clumps  = 0 ! Initalise the number of clumps to zero outside of the specific output subroutine so that it can be incremented
...
!
    ...
    restart_run: IF (file_exists .and. restart_file_read_counter == 0) then
      ...
      ! read(2,*) n_clumps_in_restart, time_in_restart_file
      ! write(*,*) n_clumps_in_restart
      ! if (n_clumps_in_restart == 0) then
      !   ! There are no clumps
      !   call specific_output(-9,-3,100)
      ! else
      !   ! Read clump information from the file and change read flag to 1
      !     do
      !       read(2,*,iostat=io) clump_id,clump_particle_ID, next_density
      !       clump_pid(clump_id) = clump_particle_ID
      !       clump_output_density(clump_id) = 10**next_density
      !       IF (io/=0) EXIT
      !     enddo
      !   write(*,*) n_clumps_in_restart, "clumps in file "
      !   write(*,*) clump_output_density(1:n_clumps_in_restart)
      !   call specific_output(-9,-3,100)
      !   restart_file_read_counter = 1
      ! endif
                                                                                                 ! momentum error because time was overwriting
    ! if (n_clumps_in_restart > 0) then                                                                                ! existing existing 'time' variable when being read
    !   restart_file_read_counter = 1                                             ! so changed the read value to time_in_restart_fie
    !   do
    !     read(2,*,iostat=io) clump_id,clump_particle_ID, next_density
    !     write(*,*) next_density
    !     clump_pid(clump_id) = clump_particle_ID
    ...
    !     IF (io/=0) EXIT
    !   enddo
    !   write(*,*) " READING CLUMP DATA "
    !   write(*,*) clump_output_density
    !   call specific_output(-9,-3,100)
    ! !
    ! !
    ! else
    ...
    ! endif restart_run
    ! ! !
    ! ! ! if (restart_file_read_counter > 0) then
    ! ! !   call specific_output(-9,-3,100)
    ! ! ! endif
    ! ! ! !
    ! fresh_start_run: IF (.not. file_exists) then
    ! !   ! The restart file hasn't been created yet so we are starting from a fresh run.
    !   call specific_output(-9,-3,100)
    ! !
    ! endif fresh_start_run
    ...
    !   DO n=1,nlines
    !     IF (n > 1) then
    !       read(1,*) clump_id,clump_particle_id,next_density
    ...
    ! Wednesday 9th Feb 2022 start
    ! if (file_exists) then
    !   call read_restart_file() ! If the restart file exists, count number of clumps recorded
    !
    !   if (n_clumps_in_restart == n_clumps) then ! If the number of clumps recorded in the restart_file
    !     call assign_values_from_restart()       ! is greater than number of clumps globally,
    !     call specific_output(-9,-4)             ! Take the values for clump_pid and next output density and assign
    !                                             ! them to the arrays before specific_output is run
    !                                             ! thereby restarting the run from where it left off
    !                                             ! Call specific output with the 'restarted' array values
    !
    !
    !   elseif (n_clumps_in_restart < n_clumps .and. n_clumps_in_restart > 0) then
    !
    !
    !     call specific_output(-9,4)        ! If the restart file only contains information on
    !                                       ! the first clump but n_clumps > 1 (i.e. a second
    !                                       ! clump has formed before the next restart file is
    !                                       ! written) then
    !
    !   endif
    ! else
    !   call specific_output(-9,-4)         ! If no restart file is found, we are starting from fresh
    !                                       ! and start the clump tracking subroutine from the beginning
    ! endif
    ! Wednesday 9th Feb 2022 end


    ! call densities(-9,-3,100)
    ! if (file_exists .and. n_clumps > 0) then
